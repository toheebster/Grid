...
struct PositioningTask: Equatable, Hashable {
  ...
  var flow: GridFlow
  private var _itemsMap: [AnyHashable?: PositionedItem]
  
  init(items: [PositionedItem], arrangement: LayoutArrangement, boundingSize: CGSize, tracks: [GridTrack], contentMode: GridContentMode, flow: GridFlow) {
    self.items = items
    ...
    self.flow = flow
    var __items = Dictionary<AnyHashable?, PositionedItem>(minimumCapacity: items.count)
  ...
  subscript(arrangedItem: ArrangedItem) -> PositionedItem? {
    ...
    _itemsMap[arrangedItem.gridElement.id]
  }
}
...
struct PositionedLayout: Equatable {
  ...
  init(items: [PositionedItem], totalSize: CGSize?) {
    self.items = items
